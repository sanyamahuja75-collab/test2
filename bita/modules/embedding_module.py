"""
Embedding Module for TGN / BiTA.
Generates dynamic node embeddings from temporal memory, node features, and temporal neighborhoods.
"""
...
class TimeEmbedding(EmbeddingModule):
    def __init__(
        ...
        self.time_projection = NormalLinear(1, self.n_time_features)
    ...
    def compute_embedding(
        ...
        if use_time_proj:
            time_projection = self.time_projection(time_diffs.unsqueeze(dim=1))
            source_embeddings = source_embeddings + time_projection
...
class GraphEmbedding(EmbeddingModule):
    ...
    def compute_embedding(
        ...
        source_node_features = (
            self.node_features[source_nodes_torch, :]
            if self.node_features is not None
            else torch.zeros((len(source_nodes), self.n_node_features), device=self.device)
        )
        ...
        if n_layers == 0:
            ...
            neighbor_embeddings = self.compute_embedding(
                ...
                n_layers=n_layers - 1,
                n_neighbors=n_neighbors,
                time_diffs=edge_deltas.flatten(),
                use_time_proj=use_time_proj,
            )
            ...
            edge_features = (
                self.edge_features[edge_idxs, :]
                if self.edge_features is not None
                else torch.zeros(
                    (len(source_nodes), effective_n_neighbors, self.n_edge_features),
                    device=self.device,
                )
            )
...
class GraphSumEmbedding(GraphEmbedding):
    def __init__(
        ...
        self.linear_1 = torch.nn.ModuleList(
            [torch.nn.Linear(embedding_dimension + n_time_features + n_edge_features, embedding_dimension)
    ...
    def aggregate(
        ...
        neighbors_features = self.linear_1[n_layer - 1](neighbors_features)
        neighbors_sum = torch.nn.functional.relu(torch.sum(neighbors_features, dim=1))
        source_features = torch.cat([source_node_features, neighbors_sum], dim=1)
        source_embedding = self.linear_2[n_layer - 1](source_features)
        return source_embedding
